...
|---|---|---|---|---|
| 1 | `title` | done | 3D title "Borderworlds:" / "Superior", drifting in, canted into the screen, on the planet's sky; then the hero flies past behind the lettering | 12.5 |
...
| 7 | `spacestation_flyby` | done | The station pass (Part C); the marauders fire along their noses, three shots hit (D-36) | 20 |
| 8 | `warp_out` | done | The hero ship gets away under the marauders' fire and warps away; they fly on for a few seconds, then warp too (D-39) | 8 |
| 9 | `asteroid_ambush` | done | Another system: the hero comes out of warp and hides behind an asteroid; the marauders arrive and pass it; the hero comes out behind them, gives chase and fires blue lasers: close misses, three hits (D-39) | 16 |
| 10 | `marauder_downfall` | done | Close-up of the marauders under blue fire; the yellow one explodes; the camera follows the green one as it warps away (D-39) | 9 |
| 11 | `hero_rolls` | done | Close-up of the hero ship flying fast, two rolls about its centreline, then straight on (D-39) | 8 |
...
- **Beams** (added 2026-09-19, D-39): every laser line (`LASER_RED` / `LASER_BLUE`) must end on a hull (within 0.05) or the ground (y = 0), or its drawn end must be out of view. A beam that ends in mid-air in the frame fails, and so does a beam that passes through a mesh object other than the one it starts on (fireballs excepted). The self-test includes a beam that stops in mid-air and one that runs through two boxes. The asset viewer may fire at nothing (`loose_beams`).
...
| 6.4 | Laser fire schedule (red bolts, near-misses) | done | Each marauder fires every 0.35 s from 12 s, guns alternating, at the player + a 1.6-unit hashed miss; beams since D-24. **Replaced 2026-09-19 (D-36)**: see the D-36 entry |
...
| 10.1 | Scene 1 `title`: layout solved on the host (left end ~5%, right end ~70%), drift-in, hold | done | `scenes/title.c`, first in the playlist. Cant 28°; the depth of the left end is solved so the ends land at 5% and 70% (scenecheck: both lines x 18..565 px). Line 1 drifts down from 0.3 s, line 2 up from 0.8 s, each over 4.5 s, then a 7.2 s hold: 12.5 s. Sky-blue PPA backdrop, no stars (D-34). Badge: 30 fps, rast 13 ms (max 19.6), ~364 textured tris; frames at 3 s and 9 s checked. 2026-09-19 (D-35): the backdrop is now the planet's sky (`PLANET_SKY_ARGB`), and from 5.6 s the hero flies past behind the lettering for 4 s: 2.4 behind the letters' front faces along the plane's normal, level between the lines, parallel to the plane, starting and ending just off screen (solved at init from the screen edges and the ship's length plus flame reach). scenecheck: ship 171×42 px, x 0..800, ≥ 0.91 clear of the letters. Badge: 30 fps throughout, rast ≤ 20.4 ms, ≤ 572 textured tris; frames at 6.2 s and 7.6 s checked |
...
| 10.4 | Scene 4 `pad_strafe` | done | 7 s. The pass comes in from the south-west, with the sun at the marauders' backs (a first version from the north-east showed only the halls' unlit sides), low over the pad at ~4.3 s, and away over the works. Red beams every 0.17 s at aim points 2.2–5 units round the landed hero, never on it; an impact burst at each. Shots: chase (outside the trailing yellow ship, the green one ahead, the pad beyond) and ground (low south-west of the pad, the pair roaring over the camera). New `scenes/flight.c`: pose on a path (facing the velocity, banking) and on a line. Planet scale: hero span 2, marauders 1.8. Badge: chase 26 fps, ground **10.6 fps** (rast 88 ms: F-28)<br>**Changed 2026-09-19 (10.10, D-39).** |
| 10.5 | Scene 5 `emergency_takeoff` | done | 8 s. Ignition with a shudder, lift-off at 1.2 s, the nose swinging from level into the climb (2–4 s); the marauders' second pass head-on and descending, crossing at 5.5 s ±2.6 units (clearance 0.77); the hero fires blue at them 4.4–5.3 s, they fire just wide (aimed fire since D-37). Shots: lift-off (low, west of the pad, the ship broadside and lit against the sky; the first angle showed it in front of the big hall, then in silhouette) and crossing (riding behind the climbing hero, the pair passing either side). Badge: lift-off 19 fps, crossing 30 fps<br>**Changed 2026-09-19 (10.10, D-39).** |
| 10.6 | Scene 8 `warp_out` | done | 7.5 s, one shot riding behind the formation (formation.c, `marauder_approach`'s slots). The hero sits 2.5 units ahead, gaining 1.5 units/s, and warps at 2.2 s; green warps at 4.8 s, yellow at 5.4 s. The station is small, far off ahead-left. Frame checks: the hero was ~20 px at first, so it was moved closer (~70 px wide from behind), and the station was moved out of the middle, where the flashes are. Badge: 30 fps, rast 3.8 ms<br>**Changed 2026-09-19 (10.10, D-39).** |
| 10.7 | Scene 9 `asteroid_ambush` | done | 16 s, second system (`scenes/system2.c`, D-38). Big rock (radius 6) at the origin plus four small ones.<br>**Hero:** warps in at 1 s on an arrival path, braking to rest behind the rock at 5 s. It emerges at 9 s: a blend from rest onto a slot 7 behind the formation, swung wide of the rock (≥ 1.99 clear). It aims its nose above the pair, sweeping across them, and fires 12.8–15.6 s; all misses.<br>**Marauders:** warp in at 5.0 / 5.4 s on its trail and pass the rock at 7.5 s, 12 units to its right.<br>**Shots:** arrival (out on the hiding side), pass (low by the rock, 3.3 from the line, turning to follow them), ambush (chase behind the hero).<br>**Frame checks:** the first arrival shot, wide from the marauders' side, showed ships of 8–12 px; that and the sun from +x (every camera saw unlit faces) led to D-38.<br>Badge: arrival/pass 30 fps; ambush 28.9 fps (rast 18.5 / 44.9 ms when the giant and the rock fill the view). Textured peak 872 at first (cap 1024), now ~840<br>**Changed 2026-09-19 (10.10, D-39).** |
| 10.8 | Scene 10 `marauder_downfall` | done | 8.8 s, one shot riding off the green ship's right. The pursuit's pair is mirrored, sways too (D-38), so the camera is on the sunlit side.<br>**Fire:** the hero, out of frame 16 behind, fires every 0.2 s: 0.6–3.4 s at both ships, 4.4–5.9 s at the green one. Misses pass 0.9–1.3 off a ship; shots 5, 9 and 12 hit the yellow ship's hull (beam ends there, sparks, a burn riding on the hull).<br>**Blast:** the yellow ship blows at 3.5 s (debris at the formation's speed, fireball 0.8 span; 1.2 was too big this close to the lens). Green climbs away from the blast (scenecheck: it touched the fireball) with the camera, and warps at 6.4 s.<br>scenecheck allows the burns on marauder#1 and one debris part through the lens. Badge: 30 fps, rast 8.6 / 25.6 ms<br>**Changed 2026-09-19 (10.10, D-39).** |
| 10.9 | Scene 11 `hero_rolls` | done | 8 s. The hero flies at 30 units/s along -z; two barrel rolls 2.2–4.8 s (roll about the nose plus a 0.35 corkscrew). The camera rides 1.9 off its sunlit right, a little ahead; from 5.8 s it falls back (16 u/s²) and the ship pulls away towards the gas giant. Space dust streaks at 0.03 s. Nearest point 1.27 from the lens. Badge: 30 fps, rast 8.3 / 20.3 ms<br>**Changed 2026-09-19 (10.10, D-39).** |
| 10.10 | The user's review of the reel, first round (D-39): the ambush's beams, the rolls' axis, the downfall's camera, the marauders' fire in the warp-out; and every other beam that breaks the rule | done | **Tooling:** `mesh_raycast()` (mesh.c) and a ray query per asset (`marauder_raycast`, `player_ship_raycast`, `asteroid_raycast`, `planet_base_raycast`), so a beam that hits stops exactly on the hull; scenecheck's beam check (C4.4), which found faults in six scenes (F-30).<br>**asteroid_ambush:** a fourth shot, `guns` (12.4 s on), ahead of the pair on the sunlit side, looking back at the hero on their tail: its beams come at the lens and leave the frame. The aim circles the green ship, then the yellow one, 1.15 off the line of fire; three shots (13.55 s green, 14.75 s and 15.35 s yellow) close onto the hull and spark. Misses are clipped against the ships and rocks. Tail slot 5.5 behind (was 7). The smallest rock was removed: the five rocks behind the pair took the textured list to 1026 (cap 1024), now 864. The hits' fireball bursts were dropped for the same reason (sparks only). Badge: guns shot 30 fps, rast 6.7 ms.<br>**hero_rolls:** the 0.35 corkscrew made the ship turn about a point beside it, near a pod (pods at ±0.43); it now rolls about its centreline (the model is centred on its origin).<br>**marauder_downfall:** the hits strike where the beam first meets the hull (the spine hit ran through the tail before). After the blast the camera stays on the green ship and, 5.3–6.2 s, swings round behind it, looking 6 units ahead along its flight: the stretch and the flash are in view. The second salvo ends at 5.2 s (was 5.9), before the swing, since from behind a miss would end mid-screen. Shots named `hits` / `blast` / `warp`.<br>**warp_out:** 8 s. The marauders fire every 0.2 s, alternately, 0–2.2 s, noses turned (bank kept) onto 0.95–1.25 past the hero, green's above it and yellow's below, so no miss crosses in front of or behind it. New `chase` shot (0–2.4 s) from the sunlit side, ~69° off the flight line, so the beams cross the screen and leave it; then the old camera behind the pair for the `warp` (hero at 3.0 s, gaining 1.2/s; green 5.3 s, yellow 5.9 s). The station moved to where the chase camera sees it. Badge: 30 fps, rast 6.1 / 2.5 ms.<br>**Also fixed (found by the check):** `emergency_takeoff`: the hero's beams went into the gap and ended mid-screen (seen from behind); its nose is now on the green ship, then the yellow one (the shot at 4.85 s is held while it swings), and every beam strikes a nose in sparks, the pods' guns converging on the aim. `spacestation_flyby`: the hit beams stopped short of the hull or ran into it; they now end where the gun line meets the hull (falling back to the ship's middle), and misses are clipped against the hero. `pad_strafe`: a beam ran through a hall to its ground spot; beams now stop at the first building in the way, the spray there.<br>Badge frames checked: ambush 12.9 / 13.58 s, downfall 5.7 / 6.6 / 6.85 s, warp_out 1.0 / 2.1 / 2.6 / 3.45 s, hero_rolls 3.0 / 3.5 s, take-off 4.48 / 5.08 s |
| **11** | **Full reel: integration** | | |
| 11.1 | Playlist = all eleven in order (C4.1); scene-to-scene continuity (liveries, directions, where the sun is); the user watches the whole reel on the badge | in progress | 2026-09-19: the playlist holds all eleven scenes in order (~112 s) and is installed on the badge for the user to watch. Continuity: in the first system both marauders' sides and the sun match between scenes 6–8. The second system shares one sky, sun and gas giant (`system2.c`); scene 10 mirrors the formation (D-38). First review round done: 10.10 (D-39) |
...
| 11.4 | MJPEG export of the full reel; the user checks the video | in progress | 2026-09-19: `make export` at `2ba0993`: 3354 frames (111.8 s) in 866 s, no errors (F-29); the live reel re-installed afterwards (`make install`). The user has since reviewed the reel (10.10), so that video predates D-39. Re-exported at `c20b2c9`: 3369 frames (112.3 s) in 875 s, no errors; waiting for the user's check of the video |
...
- **F-30** 2026-09-19, scenecheck's new beam check, first run over the reel: beams that end in mid-air in view in `asteroid_ambush` (all 70 beam-frames: the chase camera sat behind the guns, so every miss closed to its vanishing point mid-screen, as the user saw), `emergency_takeoff` (22, the hero's beams from the chase camera) and `spacestation_flyby` (6, the hit beams stopping short of the hull); beams through an object in `spacestation_flyby` (the player's hull), `marauder_downfall` (the yellow ship's tail, on the way to the spine hit) and `pad_strafe` (a hall). The pursuit's beams passed. Rule of thumb: a beam fired away from the camera can only end on something; misses need a camera that sees the line of fire at more than the half field of view (~42° across) from its axis.
...
- **D-35** 2026-09-19, user: once the lines rest, the hero ship flies past behind the lettering, left to right, parallel to it (entering at the left screen edge, leaving at the right), taking about 4 s; the title's background becomes the planet-landing sky colour (replacing D-34's sky blue). The scene length stays 12.5 s (flypast 5.6–9.6 s).
- **D-36** 2026-09-19, user: in `spacestation_flyby` the marauders' beams must run along their ships' bodies (they only fire straight ahead), and a beam must either leave the screen or end on the hero. Misses are close, and the marauders manoeuvre to aim. Two or three shots hit the hero with a small impact, to show how close it comes to dying. What was built:
  - **Flight:** from 11 s each marauder blends off its path (green by 15 s, yellow by 15.8 s) onto a track following the hero's own path 0.5 s behind (~6.6 units). The track is offset in the hero's level heading frame: green (-0.9, +1.2), yellow (+1.6, -0.8).
  - **Aim:** once on the tail, each marauder turns its nose (green 11.6–12.4 s, yellow 13–13.8 s) onto a point beside the hero on its own side. That aim point swings ±70° about that side, 1.15–1.5 units out, so the ship crabs at most ~13° against its flight path. It fires every 0.35 s from then, guns alternating.
  - **Beams:** every beam is drawn straight along the nose (`laser_submit_ray`, range now 300 for both styles), so misses run out of the frame.
  - **Hits:** green shot 6 (14.5 s), yellow shot 9 (16.95 s) and green shot 15 (17.65 s). For each, the nose swings onto the hero over ±0.35 s, the beam ends on its hull, and an impact spray plus a small fireball (0.12 span) is pinned to the hit spot in the ship's own frame, so it rolls with it. The hero also shudders for 0.5 s.
  - **Checks:** host checks show every miss passing ≥ 0.89 from the hero's centre and tail, and every beam's end off screen. scenecheck allows hero–fireball contact. Badge: reverse shot 30 fps (rast 10.6 / 18.2 ms). The flyby refs at 11 s and 17 s were re-captured.

## Verification (summary)
Automated wherever possible, via `make cycle`:
- **D-37** 2026-09-19, user: the D-36 rules for beams apply to `emergency_takeoff` and `marauder_pursuit` too. What was built:
  - **Pursuit:** each beam runs along its ship's nose, which the weave swings about, and out of the frame. The target 60 units ahead is gone.
  - **Take-off, marauders:** from 3.8 s (0.4 s before they fire) the marauders turn their noses onto a point 1.65 past the hero on their own side, and turn back after their last shot. That last shot moved from 5.2 s to 5.0 s, before the closing geometry would force a sharp turn. Their beams pass ≥ 1.5 from the hero's centre (wingtips at 1.0).
  - **Take-off, hero:** the hero turns its nose (up to ~18° off its climb) onto the middle of the gap between them, swinging ±0.4 across it at 1.3 Hz. Its blue beams pass ≥ 1.33 from a marauder's centre (wingtips at 0.9).
  - **Beam ends:** a beam heading down ends on the ground (y = 0) with an impact spray; the others run the full 300 units. A beam fired away from the camera closes to its vanishing point in the frame, as any straight line does; it cannot reach the screen edge.
  - **Checks:** frames at 4.3–5.1 s (take-off) and 1.5 / 3.87 / 5.5 s (pursuit) were checked on the badge. The pursuit refs were re-captured.
- **D-38** 2026-09-19, Claude (while building scenes 8–11; for the user's review):
  - **The second system** (`scenes/system2.c`) is the first system's starfield turned (`starfield_submit_turned`), a banded gas giant 2600 units off ahead-left of the ships' -z course, and a sun from -x (brightness 0.95). The engine light has no colour, so the "different sun" is its direction and brightness only (C4.5).
  - **The sun is on -x** because that is where scenes 9–11 put their cameras: the ambush's arrival and pass shots and the rolls' camera. With it on +x, the first frames showed every ship and the rock unlit.
  - **Scene 10 mirrors the pursuit formation**, yellow on the formation's left, so its camera can ride on the sunlit side next to the green ship.
  - **Scene 8 fires no lasers**; the warp is the point of the scene. (Replaced by D-39: the marauders fire until the cut to the warp.)
  - **Scene 9's hero fire only misses**; the hits are scene 10's.
- **D-39** 2026-09-19, user, after watching the reel:
  - `asteroid_ambush`: the hero's beams all ended in the middle of the screen; they must hit a marauder or miss it closely and leave at the screen edge (the D-36 rule, for the hero's guns).
  - `hero_rolls`: the ship barrel-rolled round one of its engine pods; it must roll about its centreline.
  - `marauder_downfall`: after the yellow ship is destroyed, the camera follows the green one so it is seen fleeing by warping away.
  - `warp_out`: the marauders keep shooting at the hero as it gets away (this replaces D-38's "no lasers in scene 8").
  - Claude applied the beam rule to every scene, made it a scenecheck check (C4.4), and fixed what that found (10.10, F-30).
