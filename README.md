...
- The textures in `textures/<segment>/` are generated by `tools/make_textures.py` (numpy +
  ...
  `app.so`.
- The app icons (`metadata/icon{16,32,64}.png`: a film reel with a "2" on it) are
  generated by `tools/make_icons.py` (Pillow); `--preview` writes a contact sheet to
  `build/icons_preview.png`.
...
then returns to the launcher. `make install` puts the live reel back.

`make export SEGMENT=craftminer` exports one act on its own (a name from `SEGMENTS`:
`space` or `craftminer`), which is how the per-act review videos are made. The same
`-DSHOWREEL_SEGMENT=` works on a normal build, where it limits what the live reel plays.
