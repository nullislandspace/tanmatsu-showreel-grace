...
The playlist (`main/reel.c`) plays in a loop:

1. **`marauder_pursuit`** (6 s) -- two marauders in formation, seen from close behind
   the right one, weaving and firing their lasers at the player's ship (out of frame).
2. **`spacestation_flyby`** (20 s) -- the player's ship threads the gap between two
...
Two more scenes are compiled in but not played: `turntable` (the player's ship on a
turntable, the original single item) and `assets` (a development viewer that orbits
each asset in turn).
