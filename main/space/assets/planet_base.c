...
void planet_base_init(void) {
    ...
    flame_init();
    s_mats[BASE_MAT_PAD]       = (mesh_mat_t){texcache_get("space/pad.png"), 0xFF96948Eu, 0};
    s_mats[BASE_MAT_MARK]      = (mesh_mat_t){NULL, 0xFFE8C020u, SE_TRI_EMISSIVE};
    s_mats[BASE_MAT_WALL]      = (mesh_mat_t){texcache_get("space/industrial_wall.png"), 0xFF767C80u, 0};
    s_mats[BASE_MAT_TANK]      = (mesh_mat_t){texcache_get("space/station_hull.png"), 0xFFC8CACCu, 0};
    s_mats[BASE_MAT_METAL]     = (mesh_mat_t){texcache_get("space/plate_gunmetal.png"), 0xFF5C626Eu, 0};
    // Unlit, like the PPA ground it has to meet (planet_base.h).
    se_texture_t const* ground = texcache_get("space/ground.png");
